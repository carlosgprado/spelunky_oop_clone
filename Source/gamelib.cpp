...
void CGameState::GotoGameState(int state, int level)
{
	game->SetGameState(state, level);
}

void CGameState::GotoGameState(int state, UI *player)
{
...
void CGame::SetGameState(int state, int level)
{
	ASSERT(state >= 0 && state < NUM_GAME_STATES);
	gameState = gameStateTable[state];
	gameState->player_p = new UI();
	gameState->OnBeginState(level);
	OnDraw();
	CSpecialEffect::SetCurrentTime();
	running = true;
}

void CGame::SetGameState(int state, UI *player)
{
	ASSERT(state >= 0 && state < NUM_GAME_STATES);
	gameState = gameStateTable[state];
	gameState->player_p = player;
	gameState->OnBeginState();
	OnDraw();
	CSpecialEffect::SetCurrentTime();
	running = true;
